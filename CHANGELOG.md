...
- **Biome adoption — Wall 7b lint gate** (`ccsc-dz8`). Adds `@biomejs/biome` as a devDep and `biome.json` with a curated rule set known already passing on the repo: `correctness/noConstAssign`, `correctness/noUnreachable(Super)`, `correctness/noInvalidConstructorSuper`, `correctness/noGlobalObjectCalls`, `security/noGlobalEval`, `suspicious/noDebugger`, `suspicious/noFocusedTests` / `noSkippedTests` (catches `.only` / `.skip` regressions), `suspicious/noDuplicate(Parameters|Case)`, `suspicious/noConstEnum`, `complexity/noExtraBooleanCast`, `complexity/noUselessConstructor`. `recommended: false` is the baseline — every rule is opt-in, so the initial PR is zero-diff on existing code. Formatter deliberately disabled (`formatter.enabled: false`) for the initial adoption; turning it on + running a one-time reformat pass is a follow-up. Wired as a required CI step in `.github/workflows/ci.yml` between `Typecheck` and `Test`. `000-docs/QUALITY_GATES.md` §7b/§7c updated from 🔴 to 🟡; rows marked for further tightening.
